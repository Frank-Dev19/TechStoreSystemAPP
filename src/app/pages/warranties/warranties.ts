...
export class WarrantiesPage implements OnInit {
  ...
  loadCoverages(): void {
    ...
    this.api.findCoverages({
      ...
      next: (response) => {
        ...
        this.coverageTotal = response.total ?? 0;
      },
  ...
  loadClaims(): void {
    this.api.findClaims({ page: this.claimPage, limit: this.limit }).subscribe({
      next: (response) => {
  ...
  get coveragePages(): number { return Math.max(1, Math.ceil(this.coverageTotal / this.limit)); }
  get claimPages(): number { return Math.max(1, Math.ceil(this.claimTotal / this.limit)); }
